...
| autoscaling\_policies\_enabled | Whether to create `aws\_autoscaling\_policy` and `aws\_cloudwatch\_metric\_alarm` resources to control Auto Scaling. | bool | `"false"` | no |
...
| container\_log\_group\_name | Log group name for the container. | string | `""` | no |
...
| fargate\_cluster\_cp | The name of the capacity provider. | list(string) | `<list>` | no |
...
| lb\_subnet | The subnet associated with the load balancer. | list(string) | `<list>` | no |
...
| scheduling\_strategy | The scheduling strategy to use for the service. The valid values are REPLICA and DAEMON. | string | `""` | no |
| security\_group\_ids | A list of associated security group IDs. | list(string) | `<list>` | no |
| service\_lb\_security\_group | The service LB security groups. | list(string) | `<list>` | no |
...
| spot\_price | The maximum hourly price you're willing to pay for the Spot Instances. | string | `""` | no |
